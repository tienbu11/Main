...
const { readdirSync, readFileSync, writeFileSync, existsSync, unlinkSync, rm ,createReadStream } = require("fs-extra");
...
const { loadDependencies, loadEnvConfig, callOnLoad } = require("./includes/moduleLoader")({ logger, listPackage, listbuiltinModules });
...
global.api = (api) => {
  ...
  var data = readFileSync(process.cwd() + "/data_api/datajson/" + api, "utf-8")
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        async function streamURL(url, type) {
            return axios.get(url, {
                ...
                const path = __dirname + `/modules/commands/cache/${Date.now()}.${type}`;
            ...
            function () {
                ...
                for (const command of listCommand) {
                    try {
                        ...
                        loadEnvConfig(module);
                        callOnLoad(module, loginApiData, botModel);
            ...
            function() {
                ...
                for (const ev of events) {
                    try {
                        ...
                        loadEnvConfig(event);
                        callOnLoad(event, loginApiData, botModel);
...
chalkAnimation.rainbow('Hoàng Quyết Tiến');
